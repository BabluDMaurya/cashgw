...
class PaymentHistoryController extends Controller
{
    public function index($id){
        try{            
            ...
            if(empty($errormes)){               
                return view('pages.user.payment-history',['user_id'=>$id,'paymenthistory'=>$totaltransaction]);  
            }else{
                Auth::logout();   
    ...
    public function archieveView(Request $request){
        try{            
            ...
            if(empty($errormes)){               
                return view('pages.user.payment-history-list',['user_id'=>$request->id,'paymenthistory'=>$totaltransaction]);  
            }else{
                Auth::logout();   
                session()->flash('status',$errormes);
                return redirect('/login');
    ...
    public function updateArchieve(Request $request){
        if(Auth::check()){
            IndividualTotalTransaction::where('id',$request->rowid)->update([ 'archieve'=>$request->status]);
            echo 'updated';
        }
    }
}
