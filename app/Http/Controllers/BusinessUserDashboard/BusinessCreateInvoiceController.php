...
use DB;
class BusinessCreateInvoiceController extends Controller {

    use 
//    PaymentHistoryTrait,
//        ActivityTrait,
        TransactionIdTrait,
        AddUserOnAddressBook,
        Notifiable,
            UnRegisterEmailTrait,
        BalanceTrait;
    ...
    public function index($id) {
        try {
            ...
            $individualKyc = BusinessKyc::where('user_id', $user->id)->first();
            $individualtwodetails = Business::select('primary_email', 'primary_phone')->where('user_id', $user->id)->first();
            ...
            if (empty($errormes)) {
                return view('pages.business.create-invoice', ['user_id' => $id, 'allCategory' => $allCategory, 'addressBookContacts' => $addressBookContacts, 'individualKyc' => $individualKyc, 'individualtwodetails' => $individualtwodetails, 'allTaxList' => $allTaxList, 'currencyMaster' => $currencyMaster]);
            } else {
                Auth::logout();
    ...
    public function CreateInvoiceList(Request $request) {
        ...
        $validator = Validator::make($request->all(), [
                    ...
                    'due_date' => 'required',
                    'bill_to_email' => 'required|email|string|exists:users,email|emailformate|emaildomain|itselfloginemail',
                    'item_name'=>'required',
                    'item_desc'=>'required',
...
//                Mail::to($request->bill_to_email)->send(new SendInvoiceMailToBillToAndCC($request->bill_to_email, $request->invoice_number, $invoice_date, $dueDate, $request->invoice_grand_total, encrypt($last_id),'business-pay-invoice'));
...
//// ------------user doesnot exits ------------////
        ...
        Mail::to($user->email)->send(new SendInvoiceMailToSender($request->bill_to_email, $request->invoice_number, $invoice_date, $dueDate, $request->invoice_grand_total));
            ...
            if ($request->cc_email != '') {
                Mail::to($request->cc_email)->send(new SendInvoiceMailToBillToAndCC($request->bill_to_email, $request->invoice_number, $invoice_date, $dueDate, $request->invoice_grand_total, encrypt($last_id),'business-pay-invoice'));
            }
            //notification 
             $notifydata = [            
...
//        echo trim(str_replace($request->hiddenCurrencySymbol, "", $request->invoice_subtotal));
//        dd($request);
//        
        $user = Auth::user();
...
//        $updateInvoiceId->invoice_number = $request->invoice_number;
    ...
    public function viewInvoicePreview(Request $request, $id) {
        ...
        $data['invoice_id'] = $id;
        $data['paynow'] = '/business-confirm-pay-invoice/';
    ...
    public function sendInvoiceOpt(Request $request, $id) {
        try {
            ...
            if (Auth::check()) {
                ...
                return redirect('/business-confirm-pay-invoice-otp/' . $id);
            } else {
    ...
    public function viewConfirmInvoicePayment(Request $request, $id) {
        ...
        $data['user_id'] = $user->id;
        $data['invoice_id'] = $id;
        $data['invoice_otp_check'] = 'business-check-invoice-otp';
        $data['manage'] = 'business-recieved-invoice';
        $data['balance'] = 'business-balance';
        $data['dashboard'] = 'businessdashboard';
