...
use Carbon\Carbon;
class BusinessManageInvoiceController extends Controller
{
...
//                    ->where('data["showdate"]','<=',Carbon::now()->toDateString())
            ...
            if(empty($errormes)){
                return view('pages.business.invoice-manage',['user_id'=>$id]);  
            }else{
                Auth::logout();   
...
//        dd($InvoiceStatusDetailsGrid);
    ...
    public function EditInvoice(Request $request,$id){
        ...
        $user = Auth::user();  
        $data['individualKyc'] = BusinessKyc::where('user_id',$user->id)->first();
        $data['individualtwodetails'] = Business::select('primary_email','primary_phone')->where('user_id',$user->id)->first();
        ...
        return view('pages.business.edit-invoice')->with($data);  
    }
...
//business information
        ...
        $data['getBusinessInfoDetails'] = $getBusinessInfoDetails;
        return view('pages.business.copy-invoice')->with($data);  
    }
    ...
    public function DeleteDraftOrCancelInvoice(Request $request){
        ...
        $link = $request->link;
        if($link == 'business-delete-draft-invoice'){
            $CreateInvoice= CreateInvoice::findorfail($invoice_id); 
            $CreateInvoice->delete(); 
            echo "1";
        }else if ($link == 'business-cancel-invoice'){
